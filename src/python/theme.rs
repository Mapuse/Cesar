...
impl ThemeEngine {
    pub fn load(cfg: &PythonConfig) -> Option<Self> {
        ...
        if !std_path.exists() {
            eprintln!("[Warning] :: theme file not found: {}", path);
            return None;
        }
        ...
        match result {
            Ok(engine) => {
                eprintln!("[Done] :: loaded theme: {}", path);
                Some(engine)
            }
            Err(e) => {
                eprintln!("[Error] :: failed to load theme {}: {}", path, e);
                None
            }
        }
    }
    ...
    pub fn run(&self) -> bool {
        Python::with_gil(|py| {
            match self.module.call_method0(py, "run") {
                Ok(_) => true,
                Err(e) => {
                    eprintln!("[Error] :: python TUI exited: {}", e);
                    false
                }
            }
        })
...
fn theme_desc_candidates() -> Vec<std::path::PathBuf> {
    ...
    if let Ok(home) = std::env::var("HOME") {
        candidates.push(std::path::PathBuf::from(home).join(".config/cesar/t.desc"));
    }
    candidates.push(std::path::PathBuf::from("/etc/cesar/t.desc"));
    ...
    ONCE.call_once(|| {
        for path in theme_desc_candidates() {
                ...
                && let Ok(config) = toml::from_str::<ThemeDescConfig>(&content)
            {
                ...
                eprintln!("[Done] :: loaded themes from t.desc: {}", path.display());
                return;
            }
        }
    });
