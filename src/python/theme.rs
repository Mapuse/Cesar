...
impl ThemeEngine {
    pub fn load(cfg: &PythonConfig) -> Option<Self> {
        ...
        if !std_path.exists() {
            eprintln!("cesar: theme file not found: {}", path);
            return None;
        }
        ...
        match result {
            Ok(engine) => {
                eprintln!("cesar: loaded theme: {}", path);
                Some(engine)
            }
            Err(e) => {
                eprintln!("cesar: failed to load theme {}: {}", path, e);
                None
            }
        }
    }
    ...
    pub fn run(&self) -> bool {
        Python::with_gil(|py| {
            match self.module.call_method0(py, "run") {
                Ok(_) => true,
                Err(e) => {
                    eprintln!("cesar: python TUI exited: {}", e);
                    false
                }
            }
        })
...
impl ThemeEngine {
    ...
    pub fn apply(entry: &ThemeEntry) -> Result<String, String> {
        let _ = entry;
        eprintln!("cesar: theme apply not available in backward-compat mode");
