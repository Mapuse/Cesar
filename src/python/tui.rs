...
impl TuiEngine {
    pub fn load(cfg: &PythonConfig) -> Option<Self> {
        ...
        if !std_path.exists() {
            eprintln!("[Warning] :: tui file not found: {}", path);
            return None;
        }
        ...
        match result {
            Ok(engine) => {
                eprintln!("[Done] :: loaded tui: {}", path);
                Some(engine)
            }
            Err(e) => {
                eprintln!("[Error] :: failed to load tui {}: {}", path, e);
                None
            }
        }
    }
    ...
    ONCE.call_once(|| {
        for path in tui_desc_candidates() {
                ...
                && let Ok(config) = toml::from_str::<TuiDescConfig>(&content)
            {
                ...
                eprintln!("[Done] :: loaded tuis from t.desc: {}", path.display());
                return;
            }
        }
    });
