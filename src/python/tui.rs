...
impl TuiEngine {
    pub fn load(cfg: &PythonConfig) -> Option<Self> {
        ...
        if !std_path.exists() {
            eprintln!("cesar: tui file not found: {}", path);
            return None;
        }
        ...
        match result {
            Ok(engine) => {
                eprintln!("cesar: loaded tui: {}", path);
                Some(engine)
            }
            Err(e) => {
                eprintln!("cesar: failed to load tui {}: {}", path, e);
                None
            }
        }
    }
...
impl TuiEngine {
    ...
    pub fn apply(entry: &TuiEntry) -> Result<String, String> {
        let _ = entry;
        eprintln!("cesar: tui apply not available in backward-compat mode");
